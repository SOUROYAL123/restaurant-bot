// ============================================
// RESTAURANT WHATSAPP BOT v5.1
// Multi-Restaurant | Razorpay Online + COD
// Full Test Mode for Local Simulation
// ============================================
  ...
  try {
    ...
    if (session.state === S.BROWSE_MENU) {
      ...
      if (upper === 'DONE') {
        ...
        if (sub < session.minOrder) { await sendMessage(phone, `⚠️ Minimum order: ₹${session.minOrder}\nCurrent: ₹${sub}`); return res.status(200).send('OK'); }
        session.subtotal = sub; session.total = sub + session.deliveryFee; session.state = S.ADD_ADDRESS;
        sessions.set(phone, session);
    ...
    if (session.state === S.ADD_ADDRESS) {
      session.deliveryAddress = text; session.state = S.ADD_INSTRUCTIONS;
      sessions.set(phone, session);
    ...
    if (session.state === S.CHOOSE_PAYMENT) {
      if (text === '1') {
        session.paymentMethod = 'online'; session.state = S.AWAITING_PAYMENT;
        sessions.set(phone, session);
        const link = await createPaymentLink(session);
        if (!link?.short_url) {
          session.state = S.CHOOSE_PAYMENT; sessions.set(phone, session);
          await sendMessage(phone, '❌ Online payment is temporarily unavailable.\n\nReply 2 for Cash on Delivery');
          return res.status(200).send('OK');
        }
        session.paymentId = link.id; session.paymentLink = link.short_url;
        sessions.set(phone, session);
      ...
      if (text === '2') {
        session.paymentMethod = 'cod'; session.state = S.CONFIRM_ORDER;
      ...
      await sendMessage(phone, '❌ Reply 1 for Online Payment or 2 for COD');
      return res.status(200).send('OK');
    }
    ...
    if (session.state === S.AWAITING_PAYMENT) {
      if (upper === 'CHECK') {
        ...
        if (paid) {
          const t = pendingPayments.get(phone); if (t) clearTimeout(t);
      ...
      if (upper === 'CANCEL') {
        const t = pendingPayments.get(phone); if (t) clearTimeout(t);
        pendingPayments.delete(phone); sessions.delete(phone);
...
app.post('/payment/webhook', async (req, res) => {
  ...
  } catch (e) { console.error('❌ Pay webhook:', e.message); res.status(500).send('Error'); }
});
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({ status:'OK', database:'Connected', sessions: sessions.size, restaurants: restaurantCache.length, testMode: TEST_MODE });
...
app.get('/test-session/:phone', (req, res) => {
  ...
  res.json(s ? { found:true, state:s.state, restaurantName:s.restaurantName, cartSize: s.cart?.length||0 } : { found:false });
});
...
app.post('/admin/clear-sessions', (req, res) => {
  ...
  sessions.clear(); res.json({ cleared: true });
});
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`
╔═══════════════════════════════════════════════╗
║   🍽️  RESTAURANT WHATSAPP BOT v5.1           ║
╠═══════════════════════════════════════════════╣
...
╠═══════════════════════════════════════════════╣
║  Trigger → Service → Menu → Address          ║
║  → Instructions → CHOOSE PAYMENT             ║
║  → [1: Razorpay Link → CHECK]                ║
║  → [2: COD → CONFIRM]                        ║
╚═══════════════════════════════════════════════╝
      `);
    });
