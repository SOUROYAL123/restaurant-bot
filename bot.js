/**
 * ═══════════════════════════════════════════════════════════
 * WHATSAPP CLINIC BOT v5.0.1 - COMPLETE WORKING VERSION
 * 
 * Fixed: Removed '1' from restart triggers
...
try {
    ...
    if (sheets && sheets.enabled !== false) GoogleSheetsLogger = sheets;
} catch (err) {
...
// HTTP Routes
app.get('/', (req, res) => res.json({ name: 'WhatsApp Clinic Bot', version: '5.0.1', status: 'operational', features: { payment_integration: true, smart_reminders: true, interactive_commands: true, waitlist: true }, uptime: Math.floor(process.uptime()) }));
...
app.get('/status', async (req, res) => { try { const s = await sql`SELECT (SELECT COUNT(*) FROM clinics WHERE status = 'active') as clinics, (SELECT COUNT(*) FROM appointments WHERE DATE(created_at) = CURRENT_DATE) as today, (SELECT COUNT(*) FROM appointments WHERE status = 'pending') as pending, (SELECT COUNT(*) FROM appointments WHERE payment_status = 'paid') as paid_deposits`; res.json({ status: 'ok', version: '5.0.1', stats: s[0] || {} }); } catch (e) { res.status(500).json({ error: 'DB unavailable' }); } });
...
async function startServer() {
    try {
        log.info('Starting v5.0.1...');
        await sql`SELECT NOW()`;
        log.success('Database connected');
        app.listen(PORT, HOST, () => {
            console.log('═══════════════════════════════════════════════════════════');
            console.log('🚀 WHATSAPP CLINIC BOT v5.0.1 - FIXED');
            console.log('═══════════════════════════════════════════════════════════');
            ...
            console.log(`🔧 Bug Fix: Clinic selection works ✅`);
            console.log('═══════════════════════════════════════════════════════════');
